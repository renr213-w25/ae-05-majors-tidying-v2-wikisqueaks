...
df <- statsci |> 
  pivot_longer(cols = "2011":last_col(), names_to = "year", values_to = "n")

```

-   **Question:** What is the type of the `year` variable? Why? What should it be?

The data type of the column is "`r typeof(df$year)`". It should be numeric.
...
# add your code here
df <- statsci |> 
  pivot_longer(cols = "2011":last_col(), 
               names_to = "year", 
               values_to = "n",
...
# add your code here
df <- statsci |>
  mutate(across("2011":last_col(), ~replace_na(., 0))) |> 
  pivot_longer(cols = "2011":last_col(), 
               names_to = "year", 
               values_to = "n",
               names_transform = list(year = as.numeric))
...
df <- statsci |>
  ...
  pivot_longer(cols = "2011":last_col(), 
               ...
               values_to = "n",
               names_transform = list(year = as.numeric)) |> 
  mutate(degree_type = str_extract(degree, "(?<=\\().+?(?=\\))"), .after = "degree")
...
df |> 
  ...
  scale_color_manual(values=c(
    ...
    "AB" = "lightgoldenrod4",
    "AB2" = "lightgoldenrod3")) +
  labs(title = "Statistical Science majors over the years",
